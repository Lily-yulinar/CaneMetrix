...
data_bj_brix = {
    ...
    17.0: 1.065410, 18.0: 1.069741, 19.0: 1.074103, 20.0: 1.078497, 
...
with st.sidebar:
    ...
    selected = option_menu(menu_title=None, options=["Dashboard", "Analisa Tetes"], 
                          icons=["house", "tree"], default_index=1) # tree icon as sugar cane alternative
    st.divider()
    st.caption("Status: 🟢 Production Ready")

# --- 4. HALAMAN ANALISA TETES ---
if selected == "Analisa Tetes":
    st.header("🎋 Analisa Tetes") # Judul diganti sesuai request
        ...
        if pol_baca > 0:
            if bj_brix:
                ...
                if purity > 100:
                    ...
                    st.success("Perhitungan Anda Berhasil") # Pesan diganti sesuai request
            else:

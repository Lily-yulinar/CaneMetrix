...
# --- 1. SETTINGS & LUXURY GLASS UI (DESIGN LOCKED) ---
...
def get_bj_icumsa(brix_val):
    """BJ Tabel ICUMSA area 8.8"""
    if brix_val <= 8.8: return 1.031047
    return 1.035950

# --- 3. HEADER DENGAN LOGO KANAN KIRI ---
if 'menu' not in st.session_state: st.session_state.menu = "main"

tz_jkt = pytz.timezone('Asia/Jakarta')
now_jkt = datetime.now(tz_jkt)

# Menggunakan 3 kolom di dalam header untuk penempatan logo
col_logo1, col_text, col_logo2 = st.columns([1, 4, 1])

with col_logo1:
    st.image("sgn.png", width=120) # Logo Kiri

with col_text:
    st.markdown(f"""
        <div style="text-align: center;">
            <h1 style="font-family:Orbitron; font-size:50px; letter-spacing:8px; margin:0; color:white;">CANE METRIX</h1>
            <p style="font-family:Lexend; color:white; opacity:0.8; letter-spacing:2px;">ACCELERATING QA PERFORMANCE</p>
            <div style="font-family:Orbitron; color:#00ced1; font-size:18px; margin-top:10px;">
...
with col_logo2:
    st.image("lpp.png", width=120) # Logo Kanan

st.markdown("---") # Garis pemisah luxury
